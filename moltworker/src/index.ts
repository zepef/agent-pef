...
import { createWebSocketProxy } from './websocket-proxy';
...
// Middleware: Log every request and warn about DEV_MODE
app.use('*', async (c, next) => {
  ...
  // Safety check: warn loudly if DEV_MODE is enabled
  if (c.env.DEV_MODE === 'true') {
    console.warn('[SECURITY] ⚠️ DEV_MODE is ENABLED - ALL authentication is bypassed!');
    // Block DEV_MODE when CF Access is configured (production indicator)
    if (c.env.CF_ACCESS_TEAM_DOMAIN && c.env.CF_ACCESS_AUD) {
      console.error('[SECURITY] DEV_MODE is enabled but CF Access is configured. This looks like a production environment.');
      console.error('[SECURITY] Remove DEV_MODE secret to restore authentication: wrangler secret delete DEV_MODE');
    }
  }
...
app.all('*', async (c) => {
  ...
  // Proxy WebSocket connections with error message interception
  if (isWebSocketRequest) {
    return createWebSocketProxy(request, sandbox, url.host);
  }
