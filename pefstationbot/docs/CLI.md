...
# 3. Check status
.\scripts\openclawd.ps1 status pefstationbot

# 4. Stop when done
.\scripts\openclawd.ps1 stop pefstationbot
```
...
```powershell
.\scripts\openclawd.ps1 health pefstationbot
```
...
.\scripts\openclawd.ps1 logs pefstationbot

# View specific log file
Get-Content "$env:USERPROFILE\.clawdbot\logs\pefstationbot\gateway.log" -Tail 50 -Wait
```
...
```powershell
.\scripts\openclawd.ps1 restart pefstationbot
```
...
```powershell
.\scripts\openclawd.ps1 start pefstationbot  # port 18791
...
.\scripts\install-openclawd-service.ps1 -Profile pefstationbot
```
...
```powershell
# Check status
nssm status openclawd-pefstationbot

# Stop service
nssm stop openclawd-pefstationbot

# Start service
nssm start openclawd-pefstationbot

# Restart service
nssm restart openclawd-pefstationbot
```
...
```powershell
# Run as Administrator
.\scripts\uninstall-openclawd-service.ps1 -Profile pefstationbot
```
...
Get-Content "$env:USERPROFILE\.clawdbot\logs\pefstationbot\orchestrator.log" -Tail 100

# Gateway log (message processing)
Get-Content "$env:USERPROFILE\.clawdbot\logs\pefstationbot\gateway.log" -Tail 100

# Tunnel log
Get-Content "$env:USERPROFILE\.clawdbot\logs\pefstationbot\tunnel.log" -Tail 50
```
...
```powershell
$env:OPENCLAWD_PROFILE = "pefstationbot"    # Default profile
...
Get-Content "$env:USERPROFILE\.clawdbot\logs\pefstationbot\orchestrator.log" -Tail 50
...
Get-Content "$env:USERPROFILE\.clawdbot\pids\pefstationbot-tunnel.url"
...
Get-Content "$env:USERPROFILE\.clawdbot\logs\pefstationbot\orchestrator.log" -Tail 30
```
...
```powershell
# Check service logs
Get-Content "$env:USERPROFILE\.clawdbot\logs\pefstationbot\service-stderr.log" -Tail 50
...
~/.clawdbot/
├── profiles/
│   ├── pefstationbot.json    # Profile config
...
│       └── ...
├── pids/
│   ├── pefstationbot-tunnel.pid
│   ├── pefstationbot-tunnel.url
│   ├── pefstationbot-gateway.pid
│   └── pefstationbot-health.pid
